return function()
    ...
    function BasicClass.new()
        return setmetatable({CleanupFunction = nil}, BasicClass)
    end
    ...
    describe('Add', function()
        ...
        it(
[[should clean up instances, objects, functions, connections, and threads]], function(
        )
    ...
    PromiseFunction('AddPromise', function()
        ...
        it('should not remove any values from the return', function()
            local NewJanitor = Janitor.new()
            local _, Value = NewJanitor:AddPromise(Promise.new(function(
                Resolve
            )
                Resolve(true)
            end)):await()
